...
% 谱分解：求解稳多项式 d(s) 使得 a(-s)a(s) + b(-s)b(s) = d(-s)d(s)
% 输入：a_coeff, b_coeff - 多项式系数向量（降幂排列）
% 输出：d_coeff - 稳定 d(s) 的系数向量
...
function H = fb_4compute_H_matrix(a,b,d,n)
    J = create_J_matrix(n);
    [L_a,U_a] = fb_3create_LU_matrices(a,n);
    [L_b,U_b] = fb_3create_LU_matrices(b,n);
    [L_d,U_d] = fb_3create_LU_matrices(d,n);
...
% 计算矩阵的特征值属性（可选择归一化方式）
% 输入：H - 方阵，normalization_type - normalize（可选）
%        'first' - 使第一个元素为1（defalut)
%        'unit' - 单位范数, everything to 1
%        'none' - 不归一化
% 输出：同上
...
% 输出：p_coeff, q_coeff - 多项式系数，e_poly - e(s)多项式
    ...
    if size(Sylvester,1) ~= 2*n || size(Sylvester,2) ~= 2*n
        error('Sylvester dimension must be: %dx%d', 2*n, 2*n);
    end
    
    if length(e) ~= n
        error('eigenvalue dimension must be: %d', n);
    end
